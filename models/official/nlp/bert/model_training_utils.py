...
def run_customized_training_loop(
  ...
  with distribution_utils.get_strategy_scope(strategy):
    ...
    token_sum_metric = tf.keras.metrics.Sum('token_count', dtype=tf.float32)
    ...
    def tf_count(t, val):
      elements_equal_to_value = tf.math.not_equal(t, val)
    ...
    def _replicated_step(inputs):
      ...
      token_sum_metric.update_state(tf_count(inputs['input_word_ids'], 1))
    ...
    while current_step < total_training_steps and not model.stop_training:
      ...
      token_count = _float_metric_value(token_sum_metric)
      current_step += steps

      # Updates training logging.
      training_status = 'Train Step: %d/%d  / loss = %s / token count = %s' % (
          current_step, total_training_steps, train_loss, token_count)
      ...
      #checkpoint every 1qtr billion tokens

      if token_count-token_checkpoint >= token_step:
        _save_checkpoint(strategy, checkpoint, model_dir,
                         "tok_"+str(token_count)+"-"+checkpoint_name.format(step=current_step))
        token_checkpoint = token_count
    ...
    training_summary = {
        ...
        'train_loss': _float_metric_value(train_loss_metric),
        'total_tokens': _float_metric_value(token_sum_metric)
    }

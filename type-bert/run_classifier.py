...
flags.DEFINE_integer('train_batch_size', 1, 'Batch size for training.')
flags.DEFINE_integer('eval_batch_size', 1, 'Batch size for evaluation.')
flags.DEFINE_string('checkpoint_path', None, 'Path to checkpoint for training')
...
def get_loss_fn(num_classes):
  ...
  def classification_loss_fn(labels, logits):
    ...
    labels_ = tf.gather_nd(labels, indices)
    logits_ = tf.gather_nd(logits, indices)

    log_probs = tf.nn.log_softmax(logits_, axis=-1)
    one_hot_labels = tf.one_hot(
        tf.cast(labels_, dtype=tf.int32), depth=num_classes, dtype=tf.float32)
...
def top1():
    def top1(labels, preds):
        """Classification loss."""
        indices = tf.where(tf.not_equal(labels, -1))
        labels_ = tf.gather_nd(labels, indices)
        preds_ = tf.gather_nd(preds, indices)
        return tf.keras.metrics.sparse_categorical_accuracy(labels_, preds_)
    top1.__name__ = 'top1'
    return top1

def top5():
    def top5(labels, preds):
        """Classification loss."""
        indices = tf.where(tf.not_equal(labels, -1))
        labels_ = tf.gather_nd(labels, indices)
        preds_ = tf.gather_nd(preds, indices)
        return tf.keras.metrics.sparse_top_k_categorical_accuracy(labels_, preds_, k=5)
    top5.__name__ = 'top5'
    return top5

def top10():
    def top10(labels, preds):
        """Classification loss."""
        indices = tf.where(tf.not_equal(labels, -1))
        labels_ = tf.gather_nd(labels, indices)
        preds_ = tf.gather_nd(preds, indices)
        return tf.keras.metrics.sparse_top_k_categorical_accuracy(labels_, preds_, k=10)
...
def get_predictions_and_labels(strategy,
  ...
  def test_step(iterator):
    ...
    def _test_step_fn(inputs):
      ...
      inputs, labels, is_user_def, filename = inputs
      start=time.time()
      logits = trained_model(inputs, training=False)
      timed_trial = time.time()-start
      # timed_trial = (end-start)/logits.shape[0]
      indices = tf.where(tf.not_equal(labels, -1))
      labels_ = tf.gather_nd(labels, indices)
      logits_ = tf.gather_nd(logits, indices)
      is_user_def_ = tf.gather_nd(is_user_def, indices)

      if not is_regression:
        probabilities = tf.nn.softmax(logits_)
        return probabilities, labels_ , is_user_def_, filename, inputs['input_word_ids'], indices, timed_trial
      else:
        return logits_, labels_, is_user_def_, filename, inputs['input_word_ids'], indices,timed_trial

    outputs, labels, is_user_def, filename, input_seq, indices, timed_trial = strategy.run(_test_step_fn, args=(next(iterator),))
    # outputs: current batch logits as a tuple of shard logits

    input_seq = tf.nest.map_structure(strategy.experimental_local_results, input_seq)
    outputs = tf.nest.map_structure(strategy.experimental_local_results, outputs)
    labels = tf.nest.map_structure(strategy.experimental_local_results, labels)
    is_user_def = tf.nest.map_structure(strategy.experimental_local_results, is_user_def)
    filename = tf.nest.map_structure(strategy.experimental_local_results, filename)
    indices = tf.nest.map_structure(strategy.experimental_local_results, indices)
    timed_trial = tf.nest.map_structure(strategy.experimental_local_results, timed_trial)
    return outputs, labels, is_user_def, filename, input_seq, indices, timed_trial

  def _run_evaluation(test_iterator):
    """Runs evaluation steps."""
    preds, golds, indices, probs, user_def_bool, times = list(), list(), list(), list(), list(), list()
    set_input_seq = list()
    file_list = list()
    indices_set = list()
    try:
      with tf.experimental.async_scope():
        while True:
          probabilities, labels, is_user_def, filename, input_seq, seq_indices, timed_trial = test_step(test_iterator)
          times.extend(list(map(float,[*timed_trial])))
          for cur_probs, cur_labels, cur_is_user_def, ainput, afilename, aindice, in zip(probabilities, labels, is_user_def, input_seq, filename, seq_indices):
            l_indices = [[] for _ in range(len(afilename))]
            for s_indices in aindice:
              i = int(s_indices.numpy()[0])
              j = int(s_indices.numpy()[1])
              l_indices[i].append(j)

            if return_probs:
              k = 100
              ind = np.argpartition(cur_probs.numpy(), -k, axis=1)[:, -k:]
              indices.append(ind)
              #indices.extend(np.argsort(cur_probs)[:,::-1][:,:100].tolist())
              m, n = ind.shape
              ss = np.reshape(np.tile(np.arange(m), (n, 1)).T, -1)
              tt = np.reshape(ind,-1)
              kk = np.column_stack((ss,tt))
              probs.append(tf.reshape(tf.gather_nd(cur_probs, kk), (m,n)).numpy())

            preds.extend(tf.math.argmax(cur_probs, axis=1).numpy())
            golds.extend(cur_labels.numpy().tolist())
            set_input_seq.extend(ainput.numpy().tolist())
            user_def_bool.extend(cur_is_user_def.numpy().tolist())
            file_list.extend(afilename.numpy().tolist())
            indices_set.extend(l_indices)

    except (StopIteration, tf.errors.OutOfRangeError):
      tf.experimental.async_clear_error()
    return (preds, golds,user_def_bool) if not return_probs else (preds, golds, indices, probs,user_def_bool, file_list, set_input_seq, indices_set, times)

  test_iter = iter(
      strategy.experimental_distribute_datasets_from_function(eval_input_fn))
  if not return_probs:
    predictions, labels, user_def_bool = _run_evaluation(test_iter)
    return predictions, labels, user_def_bool
  else:
    predictions, labels, indices, probs, user_def_bool, filelist, set_input_seq, set_indices, times = _run_evaluation(test_iter)
    return predictions, labels, indices, probs, user_def_bool, filelist, set_input_seq, set_indices, times
...
def custom_main(custom_callbacks=None, custom_metrics=None):
  ...
  if FLAGS.mode == 'predict':
    ...
    with strategy.scope():
      ...
      output_predict_file = os.path.join(FLAGS.model_dir, 'test_results_{0}.tsv'.format(ckpt))
      output_probs_file = os.path.join(FLAGS.model_dir, 'test_probs_results_{0}.nps'.format(ckpt))
      output_indices_file = os.path.join(FLAGS.model_dir, 'test_indices_results_{0}.nps'.format(ckpt))
      with tf.io.gfile.GFile(output_predict_file, 'w') as writer:
          logging.info('***** Predict results *****')
          for pred, label, is_user_def in zip(preds,labels, user_def_bool):
              output_line = '\t'.join(
                  [str(label),str(pred), str(is_user_def)]) + '\n'
